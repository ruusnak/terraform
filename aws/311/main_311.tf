...
## insert own key
	  ...
	  "sudo mkdir /opt/ibm-cloud-private-ce-3.1.1; sudo mkdir /opt/ibm-cloud-private-ce-3.1.1/cluster; cd /opt/ibm-cloud-private-ce-3.1.1",
	  "sudo docker run -e LICENSE=accept -v \"$(pwd)\":/data ibmcom/icp-inception:3.1.1 cp -r cluster /data",
...
## copy user provided key authorized_keys for ssh access and icp installation
	  ...
	  "sudo cp /tmp/id_rsa /opt/ibm-cloud-private-ce-3.1.1/cluster/ssh_key",
	  "sudo chmod +r+w /opt/ibm-cloud-private-ce-3.1.1/cluster/ssh_key",
...
## copy modified icp config & hosts file to installation directories
      "sudo cp /tmp/icphosts /opt/ibm-cloud-private-ce-3.1.1/cluster/hosts",
	  "sudo cp /tmp/config.yaml /opt/ibm-cloud-private-ce-3.1.1/cluster/config.yaml",
...
##    "sudo sed -i 's/minio: disabled/minio: disabled\n  metering: disabled\n  monitoring: disabled/' config.yaml",
      "cd /opt/ibm-cloud-private-ce-3.1.1/cluster",
      "sudo echo '*** STARTING ICP INSTALL***'",
	  "sudo docker run -e LICENSE=accept --net=host -t -v \"$(pwd)\":/installer/cluster ibmcom/icp-inception:3.1.1 install",
	  ]
  }
