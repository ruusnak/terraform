...
##      "docker pull ibmcom/icp-inception:3.1.2 && sudo docker save -o /opt/icp-inception.tar ibmcom/icp-inception:3.1.2",
...
EOF
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo '***Load ICP images from tarball***'",
      "sudo docker load -i /opt/icp-inception.tar",
      "mkdir /opt/ibm-cloud-private-ce-3.1.2; cd /opt/ibm-cloud-private-ce-3.1.2",
      ...
      "cat /tmp/id_rsa.pub >> /root/.ssh/authorized_keys",
	  "cp /tmp/id_rsa /opt/ibm-cloud-private-ce-3.1.2/cluster/ssh_key",
	  "chmod +r+w /opt/ibm-cloud-private-ce-3.1.2/cluster/ssh_key",
      ...
      "cp /tmp/icphosts /opt/ibm-cloud-private-ce-3.1.2/cluster/hosts",
	  "cp /tmp/config.yaml /opt/ibm-cloud-private-ce-3.1.2/cluster/config.yaml",
...
##    "sudo sed -i 's/minio: disabled/minio: disabled\n  metering: disabled\n  monitoring: disabled/' config.yaml",
	  ...
	  "docker run -e LICENSE=accept --net=host -t -v \"$(pwd)\":/installer/cluster ibmcom/icp-inception:3.1.2 install | sudo tee -a icpinstall.log",
	  ]
  }
