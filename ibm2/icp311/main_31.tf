...
##      "sudo docker pull ibmcom/icp-inception:3.1.1 && sudo docker save -o /opt/icp-inception.tar ibmcom/icp-inception:3.1.1",
      "docker pull ibmcom/icp-inception:3.1.1 && sudo docker save -o /opt/icp-inception.tar ibmcom/icp-inception:3.1.1",
      "echo '***Load ICP images from tarball***'",
      "sudo docker load -i /opt/icp-inception.tar",
	    "mkdir /opt/ibm-cloud-private-ce-3.1.1; cd /opt/ibm-cloud-private-ce-3.1.1",
      ...
      "cp /tmp/icphosts /opt/ibm-cloud-private-ce-3.1.1/cluster/hosts",
	  "cp /tmp/config.yaml /opt/ibm-cloud-private-ce-3.1.1/cluster/config.yaml",
...
##    "sudo sed -i 's/minio: disabled/minio: disabled\n  metering: disabled\n  monitoring: disabled/' config.yaml",
	  ...
	  "docker run -e LICENSE=accept --net=host -t -v \"$(pwd)\":/installer/cluster ibmcom/icp-inception:3.1.1 install | sudo tee -a icpinstall.log",
	  ]
  }
